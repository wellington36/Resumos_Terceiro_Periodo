...
\begin{definition}
    Dizemos que a variável aleatória X tem \textbf{distribuição uniforme no intervalo [a, b]} (denotada por $X \backsim E[a, b]$) quando sua densidade é dada por
...
\begin{corollary}
    Se $T \backsim Exp(\lambda)$ e $Y = a T$, então $Y \backsim Exp(\frac{\lambda}{a})    $$\lambda(t) = \lim_{\Delta t \rightarrow{} 0} TMF = \dfrac{F'(t)}{1 - F(t)} = - \dfrac{R'(t)}{R(t)}$$
...
\begin{definition}
    A \textbf{taxa média de falhas} de um equipamento em um instante $[t, t + \Delta t]$ é a chance de ele falhar nos próximos $\Delta t$ dado que ainda não falhou, dividido por $\Delta t$, isto é
...
\begin{corollary}
    Se X e Y são independentes, $Cov (X, Y) = 0$ e $E(X | y) = E(X)$ apara valores validos de y (isto é, sempre que $f_Y (y) \neq 0$).
\end{corollary}

\begin{definition}
    Os gráficos de $E[X|y]$ (uma função de y) e $E[Y|x]$ (uma função de x) são chamados de curvas de regressão (de X sobre y e vice-versa, respectivamente).
\end{definition}
...
\begin{theorem}
    Se $X_1, \cdots, X_n$ são independentes da distribuição $N(\mu, \sigma^2)$ então
